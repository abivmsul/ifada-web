              ...
              <div className="flex items-center justify-between h-16 md:h-20">
                ...
                <nav className="hidden md:flex items-center gap-6" aria-label="Main navigation">
                  {TOP_NAV.slice(0, 2).map((item) => (
                    <NavLink key={item.to} href={item.to} className={cn('relative px-2 py-1 text-sm transition-colors', isActive(item.to) ? 'text-white font-semibold after:absolute after:-bottom-2 after:left-0 after:w-full after:h-0.5 after:bg-white' : 'text-white/90 hover:text-white')}>
                      {item.name}
                    </NavLink>
                  ))}

                  <NavLink href="/projects" className={cn('relative px-2 py-1 text-sm transition-colors', isActive('/projects') ? 'text-white font-semibold after:absolute after:-bottom-2 after:left-0 after:w-full after:h-0.5 after:bg-white' : 'text-white/90 hover:text-white')}>
                    Projects
                  </NavLink>

                  {/* Resources dropdown */}
                  <div ref={resourcesRef} onMouseEnter={openResourcesSoon} onMouseLeave={closeResourcesSoon} onFocus={openResourcesSoon} onBlur={closeResourcesSoon} className="relative">
                    <button aria-haspopup="true" aria-expanded={resourcesOpen} onClick={() => setResourcesOpen((v) => !v)} className={cn('inline-flex items-center gap-1 px-2 py-1 text-sm rounded transition-colors', pathname.startsWith('/resources') ? 'text-white font-semibold' : 'text-white/90 hover:text-white')}>
                      <span>Resources</span>
                      <ChevronDown size={14} className={cn('transition-transform', resourcesOpen ? 'rotate-180' : 'rotate-0')} />
                    </button>

                    <div role="menu" aria-label="Resources" className={cn('pointer-events-auto absolute top-full mt-3 left-0 w-[420px] bg-white/95 text-slate-900 rounded-xl shadow-2xl ring-1 ring-black/6 backdrop-blur-md transition-all transform origin-top-left', resourcesOpen ? 'opacity-97 scale-100 translate-y-0' : 'opacity-0 scale-95 -translate-y-2 pointer-events-none')} style={{ zIndex: 60 }}>
                        ...
                        <div>
                          ...
                          <div className="mt-3 space-y-2">
                            {RESOURCES_SUB.map((sub) => (
                              <NavLink key={sub.to} href={sub.to} onNavigate={() => setResourcesOpen(false)} className="flex items-start gap-3 p-2 rounded hover:bg-slate-50">
                  ...
                  <NavLink href="/programs" className={cn('relative px-2 py-1 text-sm transition-colors', isActive('/programs') ? 'text-white font-semibold after:absolute after:-bottom-2 after:left-0 after:w-full after:h-0.5 after:bg-white' : 'text-white/90 hover:text-white')}>
                    Hadrel
                  </NavLink>
                </nav>
            ...
            <div className="fixed inset-x-0 top-0 z-50 bg-gradient-to-r from-[#06386d] to-[#0057B8] text-white/95 overflow-auto" style={{ maxHeight: '100vh' }} onClick={(e) => e.stopPropagation()}>
              <div className="px-4 pt-4 pb-6 space-y-2">
                {TOP_NAV.slice(0, 3).map((item) => (
                  <NavLink key={item.to} href={item.to} onNavigate={() => { try { mobileButtonRef.current?.click() } catch {} }} className={cn('block px-3 py-2 rounded text-white text-base', isActive(item.to) ? 'bg-white/10 font-semibold' : 'hover:bg-white/10')}>
                    {item.name}
                  </NavLink>
                ))}
                ...
                <div className="pt-2">
                  ...
                  <div>
                    <div className="w-full flex items-center justify-between px-3 py-2 rounded text-white hover:bg-white/10">
                      <span className="font-medium">Resources</span>
                    </div>

                    <div className="mt-1 space-y-1 pl-3">
                      {RESOURCES_SUB.map((s) => (
                        <NavLink key={s.to} href={s.to} onNavigate={() => { try { mobileButtonRef.current?.click() } catch {} }} className="block px-3 py-2 rounded text-white/95 hover:bg-white/10 text-sm">
                          {s.name}
                        </NavLink>
                      ))}
                    </div>
